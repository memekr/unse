...
function getDailyTarot(seed: number) {
...
export default function HomePage() {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    setTimeout(() => {
      ...
      const lrng = seededRandom((getDailySeed() + ns) * 7);
      const nums = new Set<number>();
      while (nums.size < 6) nums.add(Math.floor(lrng() * 45) + 1);

      setResult({
        name: name.trim(), zodiac, zodiacIndex, fortune, saju, tarot,
        luckyNumbers: Array.from(nums).sort((a, b) => a - b),
        birthTime,
      });
      setLoading(false);
